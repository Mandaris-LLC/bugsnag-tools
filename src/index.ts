...
commander
    ...
    .action((key, releaseStage) => {
        ...
        iOS.versionInfo().then(async (version) => {
            ...
            console.info(`Sourcemaps iOS generated successfully (${file.filePath})`);
            return file;
        }).then(async (file) => {
            await iOS.uploadSourcemap(file.filePath);
            ...
            console.info(`Sourcemaps Android generated successfully (${file.filePath})`);
            return file;
        }).then(async (file) => {
            await Android.uploadSourcemap(file.filePath);
